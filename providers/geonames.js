...
exports.geocode = function (providerOpts, loc, cbk, opts) {
  const options = _.extend({ q: loc, maxRows: 10, username: providerOpts.username || 'demo' }, opts || {});
...
exports.reverseGeocode = function (providerOpts, lat, lng, cbk, opts) {
  const options = _.extend({ lat, lng, username: providerOpts.username || 'demo' }, opts || {});
  ...
  request({
    ...
    parser.parseString(body, (err, result) => {
      ...
      if (result.geonames.address) {
        const a = result.geonames.address[0];
      ...
      if (result.geonames.geoname) {
        ...
        result.geonames.geoname.forEach((geoname) => {
      ...
      const shortNames = {};
      googlejson.results[0].address_components.forEach((c) => {
